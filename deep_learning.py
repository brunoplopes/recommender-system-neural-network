...
def para_frente_linear_ativacao(A_prev, W, b):
...
def modelo_para_frente_L(X, parameters):
    ...
    AL, cache = para_frente_linear_ativacao(A, parameters['W' + str(L)], parameters['b' + str(L)])
    caches.append(cache)
...
def modelo_para_tras_L(AL, Y, caches):
    ...
    grads["dA" + str(L)], grads["dW" + str(L)], grads["db" + str(L)] = para_tras_linear_ativacao(dAL, caches[L - 1],
                                                                                                 activation="relu")
...
dataset = pd.read_csv('dataset.csv', low_memory=False,
                      usecols=['year', 'vote_count', 'vote_average', 'popularity', 'Science Fiction', 'Action', 'War',
                               ...
                               'Crime', 'Family'])
X_assess = dataset
Y_assess = np.ones((1, X_assess.shape[1]))
layers_dims = [X_assess.shape[1],X_assess.shape[1]]
parameters = modelo_L_camadas(X_assess, Y_assess, layers_dims=layers_dims, num_iterations=100, print_cost=True)


prob, caches = modelo_para_frente_L(X_assess[:1], parameters)
